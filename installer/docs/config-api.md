...
macOS uses Homebrew for casks and formulae, and nix-darwin/Home Manager for config. Use these commands:

```bash
setup-outfitting-repo # Configure local repository location (run once)
or sync               # Apply the Homebrew cask manifest
or upgrade            # Sync manifests, then upgrade Homebrew casks
