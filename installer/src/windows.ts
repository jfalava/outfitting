...
// Route: GET /registry - Install Windows registry tweaks only
windowsApp.get("/registry", async (c) => {
  const registryScript = `# Windows Registry Tweaks Installer
# Set error action preference to stop so all errors become terminating
$ErrorActionPreference = "Stop"
$script:hasErrors = $false
