...
Public `github.com` repositories are fetched anonymously. If the repository lookup returns HTTP 404, Outfitting retries through the GitHub CLI (`gh api`), allowing private repositories when `gh` is installed and authenticated for `github.com`. Run `gh auth login --hostname github.com` and ensure the authenticated account can read the repository. A missing repository or ref can also return 404, so verify both if the authenticated retry fails. GitHub Enterprise hosts use `gh` directly; other Git remotes use Git's configured credentials.
